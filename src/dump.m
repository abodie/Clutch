...
//#ifndef __LP64__
	...
	if ((pid = fork()) == 0) {
        ...
        ERROR(@"error: Couldn't fork, did you compile with proper entitlements?");
		...
		if ((err = task_for_pid(mach_task_self(), pid, &port) != KERN_SUCCESS)) {
            ERROR(@"ERROR: Could not obtain mach port, did you sign with proper entitlements?");
		...
		for (uint32_t index = 0; index < countBlobs; index++) {
			if (CFSwapInt32(codesignblob->index[index].type) == CSSLOT_CODEDIRECTORY) { // is this the code directory?
                ...
                pages = CFSwapInt32(directory.nCodeSlots); // get the amount of codeslots
		...
		if (mach.flags & MH_PIE) {
            ...
            if (err != KERN_SUCCESS) {
                ...
                ERROR(@"ASLR is enabled and we could not identify the decrypted memory region.");
                kill(pid, SIGKILL);
                return FALSE;
            }
        }
...
BOOL dump_binary_64(FILE *origin, FILE *target, uint32_t top, NSString *originPath, NSString* finalPath) {
	...
	if ((pid = fork()) == 0) {
        ...
        ERROR(@"Couldn't fork, did you compile with proper entitlements?");
		return FALSE; // couldn't fork
	} else {
		// wait until the binary stops
		do {
...
BOOL dump64bit(FILE *origin, FILE *target, NSString *originPath, uint32_t top)
{
    fseek(target, top, SEEK_SET); // go the top of the target
	...
	if (!foundCrypt || !foundSignature || !foundStartText) {
        VERBOSE(@"dumping binary: some load commands were not found");
        NSLog(@"Crypt: %@ Sig: %@, TEXT: %@", foundCrypt, foundSignature, foundStartText);
		return FALSE;
	}
...
BOOL dump32bit(FILE *origin, FILE *target, NSString *originPath, uint32_t top)
{    
    fseek(target, top, SEEK_SET); // go the top of the target
	// we're going to be going to this position a lot so let's save it
	fpos_t topPosition;
	...
	pid_t pid; // store the process ID of the fork
	mach_port_t port = 0; // mach port used for moving virtual memory
	kern_return_t err; // any kernel return codes
	int status; // status of the wait
	//vm_size_t local_size = 0; // amount of data moved into the buffer
    mach_vm_size_t local_size = 0; // amount of data moved into the buffer
	...
	if ((pid = fork()) == 0) {
		...
		// open mach port to the other process
		if ((err = task_for_pid(mach_task_self(), pid, &port) != KERN_SUCCESS)) {
            NSLog(@"KERN_RETURN: %u", err);
            NSLog(@"mach_task_self(): %u, pid: %u, port: %u", mach_task_self(), pid, port);
            VERBOSE(@"ERROR: Could not obtain mach port, did you sign with proper entitlements?");
			kill(pid, SIGKILL); // kill the fork
			return FALSE;
		}
		...
		if (mach.flags & MH_PIE) {
            ...
            memory_object_name_t object;
            vm_region_basic_info_data_t info;
            mach_msg_type_number_t info_count = VM_REGION_BASIC_INFO_COUNT; // 32/64bit :P
            mach_vm_address_t region_start = 0;
            mach_vm_size_t region_size = 0;
            vm_region_flavor_t flavor = VM_REGION_BASIC_INFO;
            err = 0;
